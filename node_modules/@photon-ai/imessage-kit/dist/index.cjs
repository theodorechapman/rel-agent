...
var IMessageDatabase = class {
  ...
  async extractTextFromAttributedBody(attributedBody) {
    ...
    try {
      ...
      try {
        await promises.writeFile(tempFile, buffer);
...
var downloadImage = async (url, options = {}) => {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const path2 = path.join(TEMP_DIR2, `imsg_temp_${Date.now()}${ext}`);
...
var MessageSender = class {
  ...
  async resolveAttachment(path2) {
    ...
    const localPath = path.resolve(path2);
...
async function attachmentExists(attachment) {
  try {
    await fs__namespace.promises.access(attachment.path, fs__namespace.constants.F_OK);
    return true;
  } catch {
    return false;
  }
...
async function downloadAttachment(attachment, destPath) {
  ...
  await fs__namespace.promises.mkdir(destDir, { recursive: true });
  await fs__namespace.promises.copyFile(attachment.path, destPath);
}
async function getAttachmentSize(attachment) {
  try {
    const stats = await fs__namespace.promises.stat(attachment.path);
    return stats.size;
  } catch {
    return 0;
  }
...
async function getAttachmentMetadata(attachment) {
  try {
    return await fs__namespace.promises.stat(attachment.path);
  } catch {
    return null;
  }
}
async function readAttachment(attachment) {
  return await fs__namespace.promises.readFile(attachment.path);
}
function getAttachmentExtension(attachment) {
  const ext = path__namespace.extname(attachment.path);
